...
async def get(url: str = '', stream: bool = False, follow_redirects: bool = True, max_redirects: int = 30,
    ...
    return await Session(keep_alive=False).request(url=url, stream=stream, follow_redirects=follow_redirects,
                                                   max_redirects=max_redirects, decode=decode, ssl=ssl,
                                                   retries=retries, headers=headers, timeout=timeout, method='GET',
